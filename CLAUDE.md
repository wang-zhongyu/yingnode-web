...
| UI | shadcn/ui（通过 `.agents/skills/shadcn/` 初始化和管理） |
| 认证 | better-auth（通过 `.agents/skills/better-auth-*/` 配置） |
...
- **[CODING-GUIDELINES.md](CODING-GUIDELINES.md)** — 项目编码指南（1245 行）。所有代码必须符合此规范。编写任何代码前应先阅读。
- **[code-review.md](code-review.md)** — 代码审查清单。执行 `/code-review` 时必须严格按照此文件流程操作。

## 强制 Skill 调用规则

| 场景 | 必须先调用的 Skill |
|------|-------------------|
| 构建任何 UI 组件、初始化 shadcn、添加 shadcn 组件 | `.agents/skills/shadcn/` |
| 构建认证、用户、权限、组织相关功能 | `.agents/skills/better-auth-*/` |
...
以下摘自 CODING-GUIDELINES.md，高频使用：
